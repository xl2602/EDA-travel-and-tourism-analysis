...
tidy_ntto_spend_y <- raw_ntto_spend_y %>%
   ...
   mutate(Missing = ifelse(is.na(Spend), "yes", "no"), Region=tolower(Region))
...
tidy_ntto_inbound_m <- raw_ntto_inbound_m %>%
   ...
   select(Year, YearMonth, Region=key, Inbound=Arrival) %>% 
   mutate(Missing = ifelse(is.na(Inbound), "yes", "no"))

tidy_ntto_outbound_m <- raw_ntto_outbound_m %>% 
   gather(Month, Outbound, -Year, -Regions) %>% 
   mutate(Outbound=as.numeric(gsub('[^0-9]+', '', Outbound)), Month=match(Month, month.abb), Region=tolower(gsub('\\([a-zA-Z0-9]+\\)', '', Regions)), year=Year) %>% 
   mutate(Region=gsub('[0-9]+', '', Region)) %>% 
   mutate(Region=gsub("^\\s+|\\s+$", "", Region)) %>% 
   filter(!grepl("air", Region), !Region=='total overseas', !Region=='north america', !Region=='grand total') %>% 
   unite(YearMonth, year, Month, sep="/") %>% 
   select(Year, YearMonth, Region, Outbound) %>% 
   mutate(Missing = ifelse(is.na(Outbound), "yes", "no"))
...
ggplot(tidy_ntto_inbound_m, aes(x=YearMonth, y=Region, fill = Missing)) +
   geom_tile(color = "white") + 
   scale_fill_viridis(discrete = TRUE) +
...
ggplot(tidy_ntto_outbound_m, aes(x=YearMonth, y=Region, fill = Missing)) +
   geom_tile(color = "white") + 
   scale_fill_viridis(discrete = TRUE) +
