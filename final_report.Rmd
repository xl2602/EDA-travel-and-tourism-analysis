...
yearly_spend %>%
   ...
   plot_ly(x = ~Year) %>%
   add_trace(y=~`Payments (imports)`, type="scatter", name='Payments (imports)', mode = 'lines+markers', line = list(width = 1)) %>%
   add_trace(y=~`Receipts (exports)`, type="scatter", name='Receipts (exports)', mode = 'lines+markers', line = list(width = 1)) %>% 
   layout(title = "Yearly Spending", xaxis = list(title = "Year"), yaxis = list(title = "Spend"))
...
yearly_spend %>% 
   ...
   layout(title = "Receipts (exports)", xaxis = list(title = "Year"), yaxis = list(title = "Spend"))
```
...
For the future work, we could introduce more supportive data such as the inbound and outbound traveler data of other countries, and then compare it with the USA. We believe that it would reveal results that are more interesting.
